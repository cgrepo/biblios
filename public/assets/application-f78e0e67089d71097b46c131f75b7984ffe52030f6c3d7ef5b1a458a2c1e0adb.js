...
(function() {
  var ajaxCalls, checkRows, clearFrm, cryteriaEnabler, emptyFilters, emptyInput, getReturnDay, mySpin, readRents, setUp;

  $(document).on("turbolinks:load", function() {
    setUp();
    getReturnDay();
    $("label[for='returnDate']").on('dblclick', function() {
      return $('input#returnDate').attr('disabled', false);
    });
    $('.launch').on('click', function() {
      var idbooks, idsub;
      if (checkRows('subscriptors') > 0 && checkRows('books') > 0) {
        idbooks = [];
        $('.tblBHolder tbody tr').each(function() {
          return idbooks.push($(this).find('td:first').text());
        });
        ...
        return $.ajax({
          ...
          success: function(data) {
            ...
            $('.tblBorrowedBooks tbody tr').each(function() {
              var $i;
              $i = $(this).find('td:first').text();
              return $(".tblBHolder tbody tr td:contains('" + $i + "')").closest("tr").remove();
            });
        ...
        return alert('Error para guardar debe contener un usuario y al menos un libro');
      }
    });
    $('input[type=radio][name="[subscriptor_level]"]').change(function() {
      var op, url;
      ...
      return $.ajax({
        ...
        success: function(data) {
          ...
          return cryteriaEnabler();
        },
        error: function(data) {
          return alert('errore encontrados' + data);
        }
      });
    });
    $('input[type=radio][name="[cryteria_level]"]').change(function() {
      var op, url;
      ...
      return $.ajax({
        ...
        success: function(data) {
          ...
          return $('#search_left_holder').append(data);
        }
      });
    });
    $(document).on('submit', 'form#commitSearchByTitle', function(e) {
      e.preventDefault();
      if (!emptyInput($('#title'))) {
        mySpin('s');
        return $.ajax({
          ...
          success: function(data) {
            $('#modal-window').html(data);
            $('#modal-window').modal('show');
            $('.tblBooks').on('dblclick', 'tr', function() {
              if (checkRows('books') < 3) {
                ...
                $('.tblBHolder').find('thead th').css({
                  'background-color': 'rgba(0, 0, 0, 0.5)'
                });
                $(this).closest("tr").remove();
                return $('.tblBHolder tr td:last').on('dblclick', function() {
                  return $(this).closest("tr").remove();
                });
              } else {
                return alert('no se pueden agregar mas de 3 libros por Usuario');
              }
            });
            return mySpin('f');
          },
          error: function(data) {
            console.log(data);
            mySpin('f');
            return alert('El Numero de cuenta no fue encontrado');
          }
        });
      }
    });
    $(document).on('submit', 'form#commitSearchByISBN', function(e) {
      e.preventDefault();
      if (!emptyInput($('#isbn'))) {
        mySpin('s');
        return $.ajax({
          ...
          error: function(data) {
            console.log(data);
            mySpin('f');
            return alert('Datos no encontrados');
          }
        });
      }
    });
    $(document).on('submit', 'form#commitSearchByAutor', function(e) {
      e.preventDefault();
      if (!emptyInput($('#autor'))) {
        mySpin('s');
        return $.ajax({
    ...
    $(document).on('submit', 'form#commitSearchBySubAcc', function(e) {
      e.preventDefault();
      if (!emptyInput($('#account'))) {
        mySpin('s');
        return $.ajax({
          type: 'GET',
          ...
          success: function(data) {
            $('#modal-window').html(data);
            $('#modal-window').modal('show');
            $('.tblSubscriptors').on('dblclick', 'tr', function() {
              ...
              $('.tblSHolder').find('thead th').css({
                'background-color': 'rgba(0, 0, 0, 0.5)'
              });
              $(this).closest("tr").remove();
              $('.tblSHolder tr td:last').on('dblclick', function() {
                $(this).closest("tr").remove();
                return cryteriaEnabler();
              });
              return cryteriaEnabler();
    ...
    $(document).on('submit', 'form#commitSearchByName', function(e) {
      e.preventDefault();
      if (!emptyInput($('#name'))) {
        mySpin('s');
        return $.ajax({
          ...
          error: function(data) {
            console.log(data);
            mySpin('f');
            return alert('la busqueda por nombre: ' + $('#name').val() + ' no arrojo resutados');
          }
        });
      }
    });
    $('.borrowedTbl thead tr th').on('click', function() {
      var values;
      values = new Array();
      values.push($(this).text());
      if (emptyFilters()) {
        return ajaxCalls('unfiltered', values);
      } else {
        values.push($('select[name="[filtro]"]').val());
        values.push($('#name').val());
        return ajaxCalls('filtered', values);
      }
    });
    $(document).on('submit', 'form#searchData', function(e) {
      e.preventDefault();
      if (!emptyInput($('#name'))) {
        if ($('select[name="[filtro]"]').val() !== '') {
          mySpin('s');
          return $.ajax({
            type: 'GET',
          ...
          return alert('debe seleccionar por que filtrar');
        }
      }
    });
    ...
    $('label[for="filtro"]').on('click', function() {
      return $('select[name="[filtro]"]').attr('disabled', false);
    });
    return $('label[for="name"]').on('click', function() {
      return $('#name').attr('disabled', false);
    });
  });

  checkRows = function(opt) {
    if (opt === 'subscriptors') {
  ...
  getReturnDay = function() {
    var d, dayNames, rdate, rday, rmonth, ryear;
    d = new Date();
    rday = d.getDate() + 5;
    rmonth = d.getMonth() + 1;
    ryear = d.getFullYear();
    ...
    return $('input#returnDate').val($.datepicker.formatDate('dd/mm/yy ', rdate));
  };

  mySpin = function(opt) {
    if (opt === 's') {
      return $('#spinnerContainer').spin({
        lines: 9,
        length: 7,
        width: 8,
        radius: 6,
      ...
      return $('#spinnerContainer').spin(false);
    }
  };
  ...
  setUp = function() {
    ...
    return $('input[type=radio][name="[cryteria_level]"]').attr('checked', false);
  };

  cryteriaEnabler = function() {
    if (checkRows('subscriptors') === 0) {
      $('input[type=radio][name="[cryteria_level]"]').attr('disabled', true);
      return $('#search_left_holder :input').attr('disabled', true);
    } else {
      if ($('.tblSHolder tbody tr td:first').text() === '3') {
        ...
        return $('#search_left_holder :input').attr('disabled', false);
      }
    }
  };
  ...
  ajaxCalls = function(kind, values) {
    switch (kind) {
      case 'unfiltered':
        return $.ajax({
          type: 'GET',
          ...
          success: function(data) {
            return $('#borrowedTblBody').html(data);
          },
          error: function(data) {}
        });
      case 'filtered':
